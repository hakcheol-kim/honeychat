...
class PopupListViewController: BaseViewController {
    ...
    let toolBar = CToolbar.init(barItems: [.keyboardDown], itemColor: .appColor(.appColor))
...
extension PopupListViewController: UITextFieldDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        tfSearch.borderColor = .appColor(.appColor)
    }
    func textFieldDidEndEditing(_ textField: UITextField) {
        tfSearch.borderColor = UIColor.appColor(.gray221)
    }
...
extension PopupListViewController: PanModalPresentable {
    ...
    var panModalBackgroundColor: UIColor {
        return UIColor.appColor(.blackAlpa20)
    }
