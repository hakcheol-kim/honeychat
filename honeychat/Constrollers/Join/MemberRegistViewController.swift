...
class MemberRegistViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        let attrS = NSAttributedString(string: text, attributes: [.foregroundColor: UIColor.appColor(.gray125)])
    ...
    @IBAction func onClickedBtnActions(_ sender: UIButton) {
        ...
        if sender == btnGender {
            ...
            var lang = ShareData.ins.serverLanguageCode.uppercased();
...
//                http://dbdbdeep.com/site19/gate/today/join_result.php?dbdbdeep_userid=&dbdbdeep_tel=01031244920&referrer=TEST_S00259878ZC05487261&mb=Y
            ...
            param["dbdbdeep_userid"] = userName
            param["dbdbdeep_tel"] = phone
            param["mb"] = "Y"
...
//                        let userIdentifier = CipherManager.aes128EncrpytToHex(saveKeychainPlanTxt)
                        ...
                        AppsFlyerEvent.addEventLog(.signup, self.user)
                        AppsFlyerEvent.addEventLog(.joinComplete, self.user)
                        self.requestNotiYnSetting()
                        if let isAdultCheck = ShareData.ins.dfsGet(DfsKey.check19Plus) {
                            self.updateAdult()
                        }
...
extension MemberRegistViewController: UITextFieldDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        if textField == tfPhoneNumber {
            linePhoneNumber.backgroundColor = .appColor(.appColor)
        }
        else if textField == tfAuthCode {
            lineAuthCode.backgroundColor = .appColor(.appColor)
        }
        else if textField == tfNickName {
            tfNickName.borderColor = .appColor(.appColor)
        }
        else if textField == tfPartnerCode {
            tfPartnerCode.borderColor = .appColor(.appColor)
        }
    }
    func textFieldDidEndEditing(_ textField: UITextField) {
        if textField == tfPhoneNumber {
            linePhoneNumber.backgroundColor = .appColor(.gray204)
        }
        else if textField == tfAuthCode {
            lineAuthCode.backgroundColor = .appColor(.gray204)
        }
        else if textField == tfNickName {
            tfNickName.borderColor = .appColor(.gray204)
        }
        else if textField == tfPartnerCode {
            tfPartnerCode.borderColor = .appColor(.gray204)
        }
    }
}
