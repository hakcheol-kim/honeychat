...
//        #endif
    ...
    func startAuthTimer() {
        ...
        btnPhone.setTitleColor(.appColor(.appColor), for: .normal)
        btnPhone.backgroundColor = UIColor.appColor(.grayBg241)
...
//        btnPhone.setTitle("문자인증", for: .normal)
        ...
        btnPhone.backgroundColor = .appColor(.appColor)
...
//            btnPhone.setTitle("문자인증", for: .normal)
            ...
            btnAuth.backgroundColor = .appColor(.appColor)
...
//                "인증완료"
                ...
                self.btnAuth.setTitleColor(.appColor(.appColor), for: .normal)
                self.btnAuth.backgroundColor = .appColor(.gray225)
...
//                let userIdentifier = CipherManager.aes128EncrpytToHex(saveKeychainPlanTxt)
                ...
                AppsFlyerEvent.addEventLog(.login, user)
...
extension LoginViewController: UITextFieldDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        if textField == tfPhone {
            underLinePhone.backgroundColor = .appColor(.appColor)
        }
        else {
            underLineAuth.backgroundColor = .appColor(.appColor)
        }
    }
    func textFieldDidEndEditing(_ textField: UITextField) {
        if textField == tfPhone {
            underLinePhone.backgroundColor = UIColor.appColor(.gray221)
        }
        else {
            underLineAuth.backgroundColor = UIColor.appColor(.gray221)
        }
    }
}
