...
class MainViewController: MainActionViewController {
    ...
    var bannerList = [JSON]()
    var arrTabs: [MainTab] = [.video, .talk, .picture, .rank, .chat, .setting]
    ...
    override func viewDidLoad() {
        ...
        naviView.addShadow(offset: CGSize(width: 1, height: 1), color: UIColor.appColor(.blackAlpa30), raduius: 3, opacity: 0.3)
...
//            #if DEBUG
//            let vc = CertificationWebViewController.init()
//            self.navigationController?.pushViewController(vc, animated: true)
            
//            CAlertView.alertShowWithOk(title:"알림", message:"불라불라불라불라불라불라불라불라불라불라불라불라불라불라불라") { action in
//
//            }
//            let contentview = UIView()
//            contentview.backgroundColor = .orange
//            contentview.heightAnchor.constraint(equalToConstant: 100).isActive = true
//            CAlertView.showCustomContentView(contentview, btnTitles: ["cancel", "ok"]) { index in
//
//            }
//            let audltView = AdultPopupView()
//            CAlertView.showCustomView(audltView, [audltView.btnExit, audltView.btnRegist]) { index in
//                print("index : \(index)")
//            }
            
//            let vc = CertificationSFSafariViewController.init(KCB_CER_URL) { result in
//
//            }
//            vc.modalPresentationStyle = .fullScreen
//            self.present(vc, animated: true, completion: nil)
////            guard let url = URL(string: KCB_CER_URL), UIApplication.shared.canOpenURL(url) else { return }
////            UIApplication.shared.open(url, options: [:]) { result in
////                print("certification result: \(result)")
////            }
//            return
//            #endif
