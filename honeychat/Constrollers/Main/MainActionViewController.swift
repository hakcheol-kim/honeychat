...
class MainActionViewController: BaseViewController {
    ...
    private func presentCamTalkAlert() {
        ...
        vc.addAction(.cancel, NSLocalizedString("activity_txt479", comment: "취소"), UIImage(systemName: "xmark.circle.fill"), UIColor.appColor(.gray221))
        vc.addAction(.ok, NSLocalizedString("activity_txt210", comment: "음성"), UIImage(systemName: "phone.fill.arrow.up.right"), .appColor(.appColor))
        vc.addAction(.ok, NSLocalizedString("activity_txt211", comment: "영상"), UIImage(systemName: "arrow.up.right.video.fill"), .appColor(.appColor))
...
//        self.showPhoto(imgUrls: [imgUrl])
    ...
    func presentTalkMsgAlert() {
        ...
        if let bbsPoint = ShareData.ins.dfsGet(DfsKey.talkMsgOutPoint) as? NSNumber, bbsPoint.intValue > 0 {
            ...
            attr!.addAttribute(.foregroundColor, value: UIColor.appColor(.gray125), range: (result as NSString).range(of: msg2))
        }
    ...
    func sendRandomMessage(_ param: [String:Any]) {
        ApiClient.ins.requestJSON(.sendRandomMessage(param)) { result in
            let isSuccess = result["isSuccess"].stringValue
            if isSuccess == "01" {
