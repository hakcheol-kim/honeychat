...
//        heightHeight.constant = fitHeight
    ...
    func decorationUi() {
        ...
        let result = "\(pPoint) \(sPoint)"
        let attr = NSMutableAttributedString.init(string: "\(pPoint) \(sPoint)")
        attr.addAttribute(.foregroundColor, value: UIColor.appColor(.pinkParadise), range: (result as NSString).range(of: pPoint))
        attr.addAttribute(.foregroundColor, value: UIColor.appColor(.blueLight), range: (result as NSString).range(of: sPoint))
...
//        lbUserInfo.attributedText = try? NSAttributedString.init(htmlString: result)
        lbUserInfo.attributedText = attr
