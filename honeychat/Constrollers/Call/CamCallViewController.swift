...
class CamCallViewController: BaseViewController {
    ...
    let colorGreen = UIColor.appColor(.greenLight)
    let colorPurple = UIColor.appColor(.pinkMiddle)
...
//        이후 10초당 200포인트 차감
    ...
    @IBAction func onClickedBtnActions(_ sender: UIButton) {
        if sender == btnBack {
            ...
            attr.addAttribute(.foregroundColor, value: UIColor.appColor(.appColor), range: (title as NSString).range(of: tmpStr))
