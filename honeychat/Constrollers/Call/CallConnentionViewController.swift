...
class CallConnentionViewController: BaseViewController {
    ...
    var data:JSON!
    var connectedType : ConnectionType! = nil
    ...
    @IBAction func onClickedBtnActions(_ sender: UIButton) {
        if sender == btnClose {
            ...
            let from_user_sex = data["user_sex"].stringValue
            let from_user_name = data["user_name"].stringValue
            
            self.dismiss(animated: false, completion: nil)
            
            print("videocall =========================> \(data)")
            ...
            if canCall {
                ...
                param["to_user_id"] =   from_user_id //ShareData.ins.myId
                param["to_user_name"] =  from_user_name  //ShareData.ins.myName
                param["friend_mode"] = "N"
                
                ApiManager.ins.requestRandomCamCallInsertMsg(param: param) { (res) in
                    let isSuccess = res["isSuccess"].stringValue
                    if isSuccess == "01" {
                        let vc = CamCallViewController.initWithType(self.connectedType, room_key, from_user_id, user_name, self.data)
